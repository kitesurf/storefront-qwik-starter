 ...
 * Custom Cloudflare image optimization loader for use with @unpic/qwik
 * @param {string} url - The original image URL
 * @param {Object} options - Image options including width and quality
 * @returns {string} - The transformed Cloudflare URL
 */
interface ImageOptions {
  width?: number;
...
export default component$<CollectionCardProps>(({ collection }) => {
  ...
  //@ts-ignore
  const mobileSrc = cloudflareImageLoader(collection.featuredAsset?.preview, { 
    width: 480, 
    height: 480 
  });
    //@ts-ignore
  const tabletSrc = cloudflareImageLoader(collection.featuredAsset?.preview, { 
    width: 768, 
    height: 768 
  });
    //@ts-ignore
  const desktopSrc = cloudflareImageLoader(collection.featuredAsset?.preview, { 
    width: 1024, 
    height: 1024 
  });
  ...
  return (
    <LocalizedLink href={`/collections/${collection.slug}`} key={collection.id} class="block">
      <div class="group relative mx-auto w-full max-w-full sm:max-w-[480px] md:max-w-[600px] lg:max-w-[300px]">
        <div class="relative aspect-square w-full overflow-hidden">
          {/* Using responsive image with srcset and sizes */}
          <img
            src={mobileSrc}
            srcset={`
              ${mobileSrc} 480w,
